#include <assert.h>
#include <stdio.h>
#include <string.h>
#include <klee/klee.h>
#include <stdlib.h>

#include "solve.h"

#define X 0
#define Y 1
#define Z 2
#define T 3

 uint8_t remap_table[256] = {  };

void remap(uint8_t *param_1) {
    for (int i = 0; i < 16; i = i + 1) {
        param_1[i] = remap_table[( int)param_1[i]];
    }
}

void permutate(uint8_t *param_1) {
    uint8_t aux;
    aux = param_1[1];
    param_1[1] = param_1[5];
    param_1[5] = param_1[9];
    param_1[9] = param_1[13];
    param_1[13] = aux;
    aux = param_1[2];
    param_1[2] = param_1[10];
    param_1[10] = aux;
    aux = param_1[6];
    param_1[6] = param_1[14];
    param_1[14] = aux;
    aux = param_1[3];
    param_1[3] = param_1[15];
    param_1[15] = param_1[11];
    param_1[11] = param_1[7];
    param_1[7] = aux;
}

 uint8_t xor_something(uint8_t k) {
  return (unsigned int)(k >> 7) * 27 ^ k << 1;
}

void forxor( uint8_t *param_1) {
    uint8_t a, b, c; int i;
  
    for (i = 0; i < 4; i = i + 1) {
        c = param_1[i * 4 + 0];
        a = param_1[i * 4 + 0] ^
            param_1[i * 4 + 1] ^
            param_1[i * 4 + 3] ^
            param_1[i * 4 + 2];

        b = xor_something(param_1[i * 4 + 1] ^ param_1[i * 4 + 0]);
        param_1[i * 4 + 0] = param_1[i * 4 + 0] ^ b ^ a;

        b = xor_something(param_1[i * 4 + 2] ^ param_1[i * 4 + 1]);
        param_1[i * 4 + 1] = param_1[i * 4 + 1] ^ b ^ a;

        b = xor_something(param_1[i * 4 + 3] ^ param_1[i * 4 + 2]);
        param_1[i * 4 + 2] = param_1[i * 4 + 2] ^ b ^ a;

        b = xor_something(param_1[i * 4 + 3] ^ c);
        param_1[i * 4 + 3] = param_1[i * 4 + 3] ^ b ^ a;
    }
}

int main(void) {
    uint8_t block[16];
    klee_make_symbolic(block, sizeof(block), "input string");

    OPERATION(block);

    // constrain processed symbolic value to be
    // equal to the target value
    for (int i = 0; i < 16; ++i) {
        klee_assume(block[i] == target[i]);
    }

    // check for assert to see if we found a solution
    klee_assert(0);
    return 0;
}
